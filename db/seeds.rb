...
c = 0
user_hash.each do |u|
    Motel.create! name: u["name"], level: u["level"], description: u["desciption"], phone: u["phone"], zone: u["zone"], address: u["address"], latitude: u["latitude"], longitude: u["longtitude"], genre_id: u["genre_id"]
    c = c+ 1
...
user_hash.each do |u|
    ...
    r.rate= u["rate"]
    r.save!
    # Review.create! motel_id: u["motel_id"], user_id: u["user_id"], title: u["title"], content: u["content"], rate: u["rate"]
    c = c+ 1
    p c, user_hash.length
end
